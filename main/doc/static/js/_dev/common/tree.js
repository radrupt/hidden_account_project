define(function(require, exports) {
	...
	var setcurmenu = function(path) {//path必须是相对路径，即path的第一个文件夹名字必须和树的根节点名字一样，

		path = path.replace(/([\/]$)/g, "");//去除尾部斜杠
		path = path.split("/");//由于传入的path最后带有‘/’，所以实际数组多一个
		var curMenu = zTree.getNodes();
		for (var j = 0; j < path.length; j++) {
			for (var i = 0; i < curMenu.length; i++) {
				if (curMenu[i].name == "公共目录" || path[j] == curMenu[i].name) {
					curMenu = curMenu[i];
					break;
				}
			}
			if( curMenu.children == undefined ){
				zTree.expandNode(curMenu,true);
				//alert("ds");
				//return;
			}
