define(function(require, exports) {
    ...
    var init = function(setting){
        ...
        var html="<tr class='title'>"+
                ...
                "<td class='path' >"+LNG.address+"<span style='color:red'>("+LNG.absolute_path+")</span></td>"+
        ...
        for (var i in favData){
            ...
            "   <td class='path'><input type='text' id='spath' value='"
                +favData[i]['path'].replace(G.public_path,"") + "' /></td>"+
            "   <td class='action'>"+
        ...
        if (setting.substring(0,4) == 'fav&') {//如果是添加收藏
            ...
            "   <td class='name'><input type='text' id='sname' value='"+urlDecode(name)+"' /></td>"+//WD
            "   <td class='path'><input type='text' id='spath' value='"+urlDecode(path).replace(G.public_path,"")+"' /></td>"+
            "   <td class='action'>"+
    ...
    var add = function(){
        var htmltr= 
        "<tr class='favlist' name='' path=''>"+
        "   <td class='name'><input type='text' id='sname' value='df' /></td>"+
    ...
    var addSave = function(){
        ...
        var path=G.public_path + $(obj).find('#spath').val();
        if (name=='' || path ==''){
    ...
    var editSave = function(){
        ...
        var path_to=G.public_path + $(obj).find('#spath').val();
        if (name_to=='' || path_to ==''){
            tips(LNG.not_null,'error');
            return false;
        }
