define(function(require, exports) {
    ...
    var fileList_len = 1044;     //文件列表显示模式下的文件列表长度
    var fileList_name_len = 340;//文件列表下文件名字的长度
    var fileList_num_len  = 120;
    var fileList_author_len  = 120;
	...
	var _initListType = function(thistype){
		...
		if (thistype=='list') {
			...
			$('#list_type_list').html(
				"<div id='main_title' style='width:"+fileList_len+"px'>"+
				    '<div class="filetype" field="index" style="width:'+fileList_num_len+'px">'+LNG.num+'<span></span></div>'+
					'<div class="filename" field="name" style="width:'+(fileList_name_len+20)+'px">'+LNG.name+'<span></span></div>'+
				    '<div class="filetype" field="author" style="width:'+fileList_author_len+'px">'+LNG.paper_author+'<span></span></div>'+
					'<div class="filetype" field="type" >'+LNG.type+'<span></span></div>'+
	...
	var _bindHotKey = function(){
		...
		$(document).keydown(function (e){
			...
			if (Global.ctrlKey || e.keyCode == cmmand || e.ctrlKey) {
				...
				Global.ctrlKey = true;
				switch(e.keyCode){
					case 8:if( !G.public_not_admin ) ui.path.remove();isStopPP=true;break;//ctrl+backspace remove
					case 65:fileSelect.selectPos('all');break;//CTRL+A	全选
					case 8:ui.path.next();isStopPP=true;break;//前进
					case 67:if( !G.public_not_admin ) ui.path.copy();break;//CTRL+C 复制
					case 88:if( !G.public_not_admin ) ui.path.cute();break;//CTRL+X 剪切 WD
					case 83:break; 	// 屏蔽CTRL+S
					case 86:if( !G.public_not_admin ) { ui.path.past(); }break;//CTRL+V 粘贴 WD
					case 70://CTRL+F 查找
				...
				switch (e.keyCode) {
					...
					case 46:if( !G.public_not_admin ) { ui.path.remove();isStopPP=true;}break; //WD
					case 113:if( !G.public_not_admin ) { ui.path.rname();isStopPP=true;}break;//f2重命名 WD
					default:isStopPP=false;break;
				}
			}
			if (isStopPP) {
	...
	this._getFolderBox = function(list){
		var html="";
		var cursor_style = G.public_not_admin ? "style='cursor:default'" : "";
		...
		html+="<div id='"+list['name']+"' class='titleBox'><span class='title'"+ cursor_style +">"+list['name']+"</span></div></div>";
		return html;
	}	 
	//图标样式，文件模版填充
	this._getFileBox = function(list){
		...
		if (list['ext'] == 'oexe' && list['icon'] != undefined) {
			...
			html ="<div class='file fileApp menuApp' data-app="+code+" data-name='"+list.index+"-"+list.author+"-"+list.name+"' title='"
			+LNG.name+':'+list.name+"&#10;"+LNG.size+':'+list.size_friendly+"&#10;"
			+LNG.modify_time+':'+list.mtime+"'>";
			html+="<div class='ico' filetype='oexe' style='background-image:url("+icon+")'></div>";
			html+="<div id='' class='titleBox'><span class='title'"+ cursor_style +">"+display+"</span></div></div>";
			...
			html+="<div class='file fileBox menufile' data-name='"+list.index+"-"+list.author+"-"+list.name+"' title='"
			+LNG.name+':'+list.name+"&#10;"+LNG.size+':'+list.size_friendly+"&#10;"
			+LNG.modify_time+':'+list.mtime+"'>";
	...
	this._getFolderBoxList = function(list){
		var html="";
		var cursor_style = G.public_not_admin ? "style='cursor:default'" : "";
		html+="<div class='file folderBox menufolder' style='width:"+fileList_len+"px' data-name='"+list.name+"' title='"
		+LNG.name+':'+list.name+"&#10;"+LNG.modify_time+':'+list.mtime+"'>";
		html+="	<div class='filesize' style='width:"+fileList_num_len+"px'></div>";
		html+="	<div class='folder ico' filetype='folder'></div>";
		html+="	<div id='"+list['name']+"' class='titleBox' style='width:"+(fileList_name_len-10)+"px;margin-right:20px'><span class='title'"+ cursor_style +">"+list['name']+"</span></div>";
		html+="	<div class='filesize' style='width:"+fileList_num_len+"px;margin-right:20px'></div>";
	...
	this._getFileBoxList = function(list){
		...
		if (list['ext'] == 'oexe') {
			var code = urlEncode(json_encode(list));
			html ="<div class='file fileApp menuApp' style='width:"+fileList_len+"px' data-app='"+code+"' data-name='"+(list.index+"-"+list.author+"-"+list.name).replace("--","")+"' title='"
			+LNG.name+':'+list.name+"&#10;"+LNG.size+':'+list.size_friendly+"&#10;"
			+LNG.modify_time+':'+list.mtime+"'>";
		}else if (inArray(core.filetype['image'],list['ext'])) {//如果是图片，则显示缩略图，并绑定幻灯片插件
			var filePath = core.path2url(G.this_path+list['name']);
			var thumbPath = 'index.php?explorer/image&path='+urlEncode(G.this_path+list['name']);
			html+="<div picasa='"+filePath+"' thumb='"+thumbPath+"' class='picasaImage file fileBox menufile' style='width:"+fileList_len+"px' data-name='"+(list.index+"-"+list.author+"-"+list.name).replace("--","")+"' title='"+LNG.name+':'+list.name+"&#10;"+LNG.size+':'+list.size_friendly+"&#10;"
				+LNG.modify_time+':'+list.mtime+"'>";
		}else {
			html+="<div class='file fileBox menufile' style='width:"+fileList_len+"px' data-name='"+(list.index+"-"+list.author+"-"+list.name).replace("--","")+"' title='"
			+LNG.name+':'+list.name+"&#10;"+LNG.size+':'+list.size_friendly+"&#10;"
			+LNG.modify_time+':'+list.mtime+"'>";
		}
		html+="	<div class='filesize' style='width:"+fileList_num_len+"px' title='"+LNG.num+':'+list['index']+"'>"+ list['index']+"</div>";
		html+="	<div class='"+list['ext']+" ico' filetype='"+list['ext']+"' ></div>";	
		html+="	<div id='"+list['name']+"' class='titleBox' style='width:"+(fileList_name_len-10)+"px;margin-right:20px'><span class='title'"+ cursor_style +">"+ list['name']+"</span></div>";

		html+="	<div class='filesize' title='"+LNG.paper_author+':'+ list['author']+"'style='width:"+fileList_num_len+"px;margin-right:20px' >"+ list['author']+"</div>";
	...
	var _f5 = function(is_data_server,is_animate,callback) {
		...
		if(!is_data_server){//采用当前数据刷新,用于显示模式更换
			...
			$.ajax({
				url:'index.php?explorer/pathList&path='+urlEncode(G.this_path),//这里的path的值只是用作记录用户的操作历史的吗？
				dataType:'json',
				beforeSend:function(){
	...
	return{	
		...
		init:function(){
			...
			Mousetrap.bind(['alt+n', 'alt+n'],function(e){
				stopPP(e);if( !G.public_not_admin ) ui.path.newFile();
	        });
			Mousetrap.bind(['alt+m', 'alt+m'],function(e){
				stopPP(e);if( !G.public_not_admin ) ui.path.newFolder();
	        });	        
