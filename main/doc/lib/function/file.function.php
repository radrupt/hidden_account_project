...
function file_info($path,$date_formate=false){
	...
	$info = explode('-', $name);//文件夹名不可以带'-'
	$title = $info[count($info)-1];
	$author = (count($info) < 2) ? "" : $info[count($info) - 2];
	$index = '';
	...
	$info = array(
		'index'			=> iconv_app($index),
		'name'			=> iconv_app($title),
		'author'			=> iconv_app($author),
...
function folder_info($path,$date_formate=false){
	...
	if (!$date_formate) $date_formate = $GLOBALS['L']['time_type'];
	$public_folder = strpos($path,'public/') == false ? 'false' : 'true';//该文件夹是否是public下的文件夹

	$info = array(
		'child_num'		=> iconv_app(get_child_num($path)),
		'name'			=> iconv_app(get_path_this($path)),
		'path'			=> iconv_app(get_path_father($path)),
		'public_folder' => iconv_app($public_folder),
		'type' 			=> 'folder',
		'mode'			=> get_mode($path),
		'atime'			=> date($date_formate, fileatime($path)), //访问时间
		'ctime'			=> date($date_formate, filectime($path)), //创建时间
...
function path_list($dir,$list_file=true,$check_children=false){
	$dir = rtrim($dir,'/').'/';//去除路径尾部的‘/’
	if (!is_dir($dir) || !($dh = opendir($dir))){//不是目录或是目录但无法打开，则返回
...
//获取文件夹下的文件数量
function get_child_num($dir){
	$dir = rtrim($dir,'/').'/';
	if (!$dh = @opendir($dir)) return '';//是文件
	$num = 0;
	while (($file = readdir($dh)) !== false){
		if ($file != "." && $file != "..") {
			$fullpath = $dir.$file;
			if(is_dir($fullpath.'/')){
				$childnum = get_child_num($fullpath);
				if ($childnum != '') $num += $childnum;
			}
			if(is_file($fullpath)){
				$num += 1;
			}
...
function upload($fileInput, $path = './'){
	...
	if( $_SESSION['kod_user']['name'] == 'xu.c' ){
    	$file1 = fopen(DATA_PATH."maildata.txt","a");
    	$filepathrelatives =explode("/",iconv("gbk", "UTF-8",str_replace(DATA_PATH.'public/',"",$path).$file_name));
    
        if(count($filepathrelatives) == 2){
            $filepath = "<pre><strong style='color:red'>".'   '.$filepathrelatives[0]."</strong>"."新增: "
                    ."<strong style='color:blue'>".$filepathrelatives[1]."</strong></pre>";
        	echo fwrite($file1,$filepath."\n");
        }else if(count($filepathrelatives) == 3){
            $filepath = "<pre><strong style='color:red'>".'   '.$filepathrelatives[1]."</strong>"."新增: "
                    ."<strong style='color:blue'>".$filepathrelatives[2]."</strong></pre>";
        	echo fwrite($file1,$filepath."\n");
        }
