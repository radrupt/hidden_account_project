...
class   explorer extends Controller{
    ...
    public function index(){
        if(isset($this->in['path']) && $this->in['path'] !=''){
            ...
            $dir = PUBLIC_PATH.' 术有专攻/Android/';//首次进入系统,不带参数
       //     $dir = PUBLIC_PATH.'/Android/';//首次进入系统,不带参数
    ...
    public function pathList(){
        ...
        $_SESSION['this_path']=$user_path;//记录当前的路径，$_SESSION：服务器上存储用户信息以便随后使用
        $list=$this->path($this->path);//list文件夹里的信息
    ...
    private function _tree_init($app){
        ...
        if ($check_file) {//编辑器
            ...
            $root_isparent = count($root)>0?true:false;
            $share_isparent = count($share)>0?true:false;
            $child_num = get_child_num(PUBLIC_PATH);
            $tree_data = array(
                array('name'=>$this->L['fav'],'ext'=>'__fav__','iconSkin'=>"fav",
                    'open'=>true,'children'=>$fav),
//                 array('name'=>$this->L['root_path'],'ext'=>'__root__','children'=>$root,
//                     'iconSkin'=>"my",'open'=>true,'this_path'=> MYHOME,'isParent'=>$root_isparent),
                array('name'=>$this->L['public_path'],'ext'=>'__root__','children'=>$share,'child_num'=>$child_num,'public_folder'=>'true',
                    'iconSkin'=>"lib",'open'=>true,'this_path'=> PUBLIC_PATH,'isParent'=>$share_isparent)
            );
        }
        show_json($tree_data);
    ...
    public function sendEmail(){
    	$mess = file_get_contents(DATA_PATH."maildata.txt");
    	if( $mess === "" ) show_json('当前无新增文件.',1);
    	else {
        	$mess = "Dear All,\n" . "软件所文档管理网站新增文档:\n" . $mess . "\n谢谢!";
        	$mess = nl2br($mess);
    		...
    		$mail->CharSet    ="UTF-8";
    		$mail->IsSMTP();  
    		$mail->Host       = "mail.tcl.com";
    		$mail->Port       = 25;
    		$mail->SMTPAuth = true;
    		$mail->SMTPSecure = "none";
    		$mail->Username = "xu.c";
    		$mail->Password = "xuchao2013#";
    		$mail->SetFrom('xu.c@tcl.com', '[SW]徐超');    // 设置发件人地址和名称
    		
    		$mail->Subject    = '软件所新增文档';                     // 设置邮件标题
    		$mail->isHTML( true );
    		$mail->MsgHTML($mess);                         // 设置邮件内容
    		$recipients = array(
		      'xu.c@tcl.com' => '[SW]徐超',
              'cdl@tcl.com' => '[SW]陈德林',
              'cxiong@tcl.com' => '[SW]陈雄',
              'daijl@tcl.com' => '[SW]戴俊林',
              'hefang@tcl.com' => '[SW]何方',
              'hewl@tcl.com' => '[SW]何文亮(Ivan)',
              'huangfh@tcl.com' => '[SW]黄锋华',
              'huanghaim@tcl.com' => '[SW]黄海明(Hugo)',
              'wp2.huangll@tcl.com' => '[SW]黄林亮',
              'huxf@tcl.com' => '[SW]胡循锋',
              'jiangmc@tcl.com' => '[SW]蒋密纯',
              'kangde@tcl.com' => '[SW]康德恩(Deen)',
              'liangyp@tcl.com' => '[SW]梁郁沛',
              'lichjin@tcl.com' => ' [SW]李楚进',
              'liluyuan@tcl.com' => '[SW]李璐元',
              'liuguanglin@tcl.com' => '[SW]刘光林',
              'Liuxx@tcl.com' => '[SW]刘晓晓(kiki)',
              'liuxu@tcl.com' => '[SW]刘旭',
              'longfy@tcl.com' => '[SW]龙飞扬',
              'longyingchun@tcl.com' => '[SW]龙迎春',
              'luofg@tcl.com' => '[SW]罗富贵',
              'pangwz@tcl.com' => '[SW]庞伟振',
              'sunkk@tcl.com' => '[SW]孙侃侃',
              'tq@tcl.com' => '[SW]唐庆',
              'wangsonglin@tcl.com' => '[SW]汪松林',
              'xiehh@tcl.com' => '[SW]谢挥宏',
              'xjx@tcl.com' => '[SW]谢家勋',
              'yuansaichun@tcl.com' => '[SW]袁赛春',
              'zhangyang@tcl.com' => '[SW]张阳',
              'zhaolong@tcl.com' => '[SW]赵龙',
              'zfl@tcl.com' => '[SW]朱飞龙',
              'qiuhailang@tcl.com' => '[SW]邱海浪',
              'w.di@tcl.com' => '[SW]王迪',
              'yj@tcl.com' => '[SW]杨佳',
              'yang.yong@tcl.com' => '[SW]杨勇',
              'zhou.peng@tcl.com' => '[SW]周朋',
              'zhuke@tcl.com' => '[SW]朱珂',
              'chenqx@tcl.com' => '[SW]陈秋香',
            );
            foreach($recipients as $email => $name)
            {
               $mail->AddAddress($email, $name);
            }
            $mail->AddCC("wp.lixl@tcl.com","[SW]李晓亮(Hunter)");
            //$mail->AddCC("962889483@qq.com");
    		$mail->SMTPDebug = 0;
    		if( $mail->Send() ) {//邮件发送成功，清空保存数据 
    		
    			$file1 = fopen(DATA_PATH."maildata.txt","w");
    			echo fwrite($file1,"");
    			fclose($file1);
    		}else{
    		     show_json('邮件发送失败.',2);
